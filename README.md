...
+ `spiders/ruubypay` 从[亿通行](https://web.ruubypay.com/web/bj-subway-map-h5-output/#/map)接口获取数据的爬虫。**不建议**您运行这些文件。
+ `spiders/osm` 从 [Open Street Map (OSM)](https://www.openstreetmap.org/) 获取数据的脚本。
+ `spiders/amap` 从[高德开放平台](https://lbs.amap.com/)获取数据的爬虫。
+ `data` 原始或中间数据。
+ `tools` 用于数据分析整理转换的脚本。
+ `gtfs` （未打包的）GTFS 格式数据。
...
本项目大量采用了 OSM 的数据（以及作者顺手修复了些错误）和少部分高德地图的数据。这里提供一个简明图表来体现各数据的关联。
...
+ 对于 `location_type=0` ，即站台，是列车的**停车点**。 `stop_id` 为 OSM 中标签包含 `{"public_transport": "stop_area", "type": "public_transport"}` 的关系的ID。其中换乘站不同线路对应不同的`stop_id`。特别地，*6号线海淀五路居站*（未使用）、*5号线宋家庄站*、*13号线西直门站*为西班牙式月台，本数据集仅包含停车股道而不包含上下车月台信息，参考[西班牙式月台](https://zh.wikipedia.org/wiki/%E8%A5%BF%E7%8F%AD%E7%89%99%E5%BC%8F%E6%9C%88%E5%8F%B0#%E5%8C%97%E4%BA%AC%E5%9C%B0%E9%93%81)。
...
+ `trip_id`，虚拟车次，由本项目定义。规则为第1-2位为 `route_id`，第3位为**服务日标志位**，第4位为`direction_id`，第5位为**行程标志位**，第6-8位为**行程编号**。其中**服务日标志位**和 `service_id` 对应，对于一般线路为 `wd => 0, we => 1`，对于*首都机场线*为 `mon_thu+sat_sun => 0, fri => 1`，对于*大兴机场线*为 `mon_thu+sat => 0, fri => 1, sun => 2`；**行程标志位**对于全程车（含完整环线）取 `0`、区间车（含不完整环线）取 `1`、大站快车取 `2`、需待避慢车取 `3`、跨线车取 `4`、支线车（暂无）取 `5`；当前项完全一致时，**行程标编号**由 `001` 起按照发车时间先后递增。
...
+ 键或值使用`行内代码`，具体例使用*斜体*，关键表达和标准规范文件使用**粗体**。
+ OSM 相关术语：node-节点 relation-关系 way-路径 area-区域 member-成员 ref-引用 platform-月台 stop_area-停车区 stop_position-停车点。
